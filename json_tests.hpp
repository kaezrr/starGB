...
bool match_cpu_data(GameBoy& gb, json& data, std::ostream& out) {
    ...
    if (gb.sm83.AF.lo != static_cast<u8>(data["f"])) {
        print_error(out, "F register mismatch", data["f"], gb.sm83.AF.lo);
        return false;
    }
    if (gb.sm83.BC.hi != static_cast<u8>(data["b"])) {
        print_error(out, "B register mismatch", data["b"], gb.sm83.BC.hi);
        return false;
    }
    if (gb.sm83.BC.lo != static_cast<u8>(data["c"])) {
        print_error(out, "C register mismatch", data["c"], gb.sm83.BC.lo);
        return false;
    }
    if (gb.sm83.DE.hi != static_cast<u8>(data["d"])) {
        print_error(out, "D register mismatch", data["d"], gb.sm83.DE.hi);
        return false;
    }
    if (gb.sm83.DE.lo != static_cast<u8>(data["e"])) {
        print_error(out, "E register mismatch", data["e"], gb.sm83.DE.lo);
        return false;
    }
    if (gb.sm83.HL.hi != static_cast<u8>(data["h"])) {
        print_error(out, "H register mismatch", data["h"], gb.sm83.HL.hi);
        return false;
    }
    if (gb.sm83.HL.lo != static_cast<u8>(data["l"])) {
        print_error(out, "L register mismatch", data["l"], gb.sm83.HL.lo);
        return false;
    }
    if (gb.sm83.PC.full != static_cast<u16>(data["pc"])) {
        print_error(out, "program counter mismatch", data["pc"], gb.sm83.PC.hi);
        return false;
    }
    if (gb.sm83.SP.full != static_cast<u16>(data["sp"])) {
        print_error(out, "stack pointer mismatch", data["sp"], gb.sm83.SP.hi);
        return false;
    }
...
bool run_test(json& data, GameBoy& gb, std::ostream& out) {
    ...
    for (const auto& c : data["cycles"]) {
        ...
        bool check = c.is_null() ? s == "null" : (c[0] == a && c[1] == d && c[2].get<string>() == s);

        if (check) { ++cycle; continue; }
...
void run_all_tests(const string& folder, GameBoy& gb, std::ostream& out) {
    for (int i = 0x00; i <= 0xff; ++i) {
        switch (i) { // Ignore invalid opcodes, stop, ei, di and halt.
        case 0xD3:
        case 0xDB:
        case 0xDD:
        case 0xE3:
