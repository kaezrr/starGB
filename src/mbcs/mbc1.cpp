...
u8 MBC1::read_rom(u16 at) const {
    if(at <= 0x3FFF) { // rom bank 0
        if(!mode_flag) return rom_banks[at];
        u16 addr = 0;
        switch (rom_banks.size()) {
        case (0x4000 * 0x40):
            addr = (ram_num & 0x1) << 5; break;
        case (0x4000 * 0x80):
            addr = (ram_num & 0x3) << 5; break;
        }
    ...
    if(at <= 0x7FFF) { // rom bank ...
        ...
        switch (rom_banks.size()) {
        case (0x4000 * 0x40):
            addr = ((rom_num & 0x1F) & ~(0x20)) | ((ram_num & 0x1) << 5); break;
        case (0x4000 * 0x80):
            addr = ((rom_num & 0x1F) & ~(0x60)) | ((ram_num & 0x3) << 5); break;
        }
...
u8 MBC1::read_ram(u16 at) const {
    ...
    case (0x2000):
        ...
        break;
    default:
        if (mode_flag) addr = 0x2000 * ram_num + (at - 0xA000);
        else addr = (at - 0xA000);
    }
