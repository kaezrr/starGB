...
void CPU::stop() { return; }

void CPU::halt() { halt_mode = true; }
...
void CPU::ld_r16mem_a() { write_r16mem(op.bits(Masks::b54), AF.hi); }

void CPU::ld_a_r16mem() { AF.hi = read_r16mem(op.bits(Masks::b54)); }
...
void CPU::inc_r16() {
    tick();
    write_r16(op.bits(Masks::b54), read_r16(op.bits(Masks::b54), false) + 1,
              false);
}
...
void CPU::ld_r8_imm8() { write_r8(op.bits(Masks::b543), get_nextu8()); }
...
void CPU::ldh_c_a() { write_mem(0xFF00 + BC.lo, AF.hi); }
...
void CPU::ldh_a_c() { AF.hi = read_mem(0xFF00 + BC.lo); }
...
void CPU::jp_hl() { PC.full = HL.full; }
