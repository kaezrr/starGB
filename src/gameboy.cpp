...
void GameBoy::start() {
    ppu.screen.init("StarGB", SCREEN_HEIGHT, SCREEN_WIDTH, 3, 0xA1EF8C,
                    0x3FAC95, 0x446176, 0x2C2137);
    debugger.tiles.init("VRAM Viewer", TILE_HEIGHT * TILE_WINDOW_HEIGHT,
                        TILE_WIDTH * TILE_WINDOW_WIDTH, 2, 0xA1EF8C, 0x3FAC95,
                        0x446176, 0x2C2137, 436, 324);
    ...
    while (enabled) { // Loop runs at 59.7 Hz
        ...
        while (sm83.elapsed_cycles <
               17556) { // 17556 cycles per frame ~ 4.19MHz
            if (debugger.enabled)
                debugger.write_text_log();
            run_instruction();
