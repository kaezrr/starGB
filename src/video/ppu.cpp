...
void PPU::increment_ly() {
    ...
    if (fetcher.ly == fetcher.wy) fetcher.wy_cond = true;
    eval_lyc_intr();
}

void PPU::eval_lyc_intr() {
    if(!(fetcher.stat & 0x40)) return;
...
void PPU::drawing() {
    ...
    if (fetcher.x_pos >= 160) {
        ...
        if (fetcher.stat & 0x08) req_interrupt(LCD);
    }
}
...
void PPU::hblank() {
    ...
    if (fetcher.ly == 144) {
        ...
        if (fetcher.stat & 0x10) req_interrupt(LCD);
    } else {
        new_line();
        mode = PPU_State::OAM_SCAN;
        if (fetcher.stat & 0x20) req_interrupt(LCD);
        curr_sprite_location = OAM_S;
    }
}
...
bool PPU::push_to_display() {
    ...
    u16 bg_pixel = (fetcher.lcdc & 1) ? (fetcher.queue_bg >> 14) : 0; // pop 2 bits from the fifo
    fetcher.queue_bg <<= 2; --fetcher.bg_count;
    u16 select = bg_pixel, palette = fetcher.bgp; // select appropriate color from palette

    if (fetcher.sp_count) {
        u16 sp_pixel = (fetcher.queue_sp >> 28);
        fetcher.queue_sp <<= 4; --fetcher.sp_count;
        if ((fetcher.lcdc & 2) && (sp_pixel >> 2) && !((sp_pixel & 1) && bg_pixel)) {
            select = sp_pixel >> 2;
...
void PPU::write(u16 at, u8 data) {
    switch (at) {
    case LCDC:
        fetcher.lcdc = data; return;
    case STAT:
        update_read_only(fetcher.stat, data | 0x80, 0x07);
        return;
    case SCY:
        fetcher.scy = data; return;
    case SCX:
    ...
    case DMA:
        fetcher.dma = data; return;
    case BGP:
        fetcher.bgp = data; return;
    case OBP0:
        fetcher.obp0 = data; return;
    case OBP1:
        fetcher.obp1 = data; return;
    case WY:
        fetcher.wy = data; return;
    case WX:
        fetcher.wx = data; return;
    }
}